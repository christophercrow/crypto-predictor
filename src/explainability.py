...
def main():
    ...
    # shap_values[0] corresponds to the model output;
    # we extract its first (and only) element.
    ...
    # Optionally adjust expected_value if it's a list (KernelExplainer may return a list)
    expected_value = explainer.expected_value
    if isinstance(expected_value, (list, np.ndarray)):
        # If it's a list with multiple entries, select the first one.
